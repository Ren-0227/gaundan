...
def main():
    ...
    model = MLPActorCritic((2, 513 + ActionNumber * 54), ActionNumber).to(device)
    with open('./ppo.pth', 'rb') as f:
        new_weights = CPU_Unpickler(f).load()
    model.set_weights(new_weights)
    agent = PPOAgent(model)
    model_update_times = 0
    with open(f'/data/biaoweilin/guandan/torch_version/danzero_torch/danzero_plus/learner_torch/ckpt_bak/{model_update_times}.pth', 'wb') as f:
        ...
        if len(mem_pool) >= args.batch_size:
            ...
            if args.keep_training:
                ...
                if model_update_times > 4:
                    os.remove(f'/data/biaoweilin/guandan/torch_version/danzero_torch/danzero_plus/learner_torch/ckpt_bak/{model_update_times - 5}.pth')
                # print(stat)
                # if log_times%1000 == 0:
                if log_times%100 == 0:
