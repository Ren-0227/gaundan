...
class MCAgent(Agent):
    def __init__(self, model_cls, observation_space, action_space, config=None, lr=1e-5, 
                 *args, **kwargs):
        # Define parameters
        self.lr = lr
        self.lamda = 0.65
    ...
    def build(self) -> None:
        ...
        # cliped_q = tf.clip_by_value(self.policy_model.values / self.old_q, 1-self.lamda, 1+self.lamda) * self.old_q
        # self.loss = tf.reduce_mean((cliped_q - self.target_ph) ** 2)
        
        self.loss = tf.reduce_mean((self.policy_model.values - self.target_ph) ** 2)
        # Create the optimizer
        optimizer = tf.train.RMSPropOptimizer(learning_rate=self.lr, epsilon=1e-5)
                    
        # Compute gradients
        gradients, variables = zip(*optimizer.compute_gradients(self.loss))
            
        # Clip gradients by value (clip all gradients to a range of -0.5 to 0.5)
        clipped_gradients = [tf.clip_by_value(grad, -0.5, 0.5) if grad is not None else None 
                                         for grad in gradients]
            
        # Apply the clipped gradients
        self.train_q = optimizer.apply_gradients(zip(clipped_gradients, variables))
