    ...
    async def handle_action_request(self, msg_data):
        ...
        try:
            ...
            if not actions or len(actions) == 1:
                ...
                # 取top2最大值及其索引
                top2_vals, top2_indices = torch.topk(q_preds, 2)

                temperature = 0.02  # 你可以调整这个值，比如0.1~1.0
                prob = F.softmax(top2_vals / temperature, dim=0)

                if random.random() < prob[0]:
                    action_index = actions[top2_indices[0].item()]["index"]
                else:
                    action_index = actions[top2_indices[1].item()]["index"]
                
                self.logger.info(f"{top2_vals = } {prob = }")
