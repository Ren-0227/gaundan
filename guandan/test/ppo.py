...
class GDTestClient:
    def __init__(self, key):
        ...
        # # 使用key初始化独立的日志记录器
        self.model_q.load_state_dict(torch.load("q_network.pth"))
    ...
    async def handle_action_request(self, msg_data):
        ...
        try:
            ...
            if not actions or len(actions) == 1:
                ...
                with torch.no_grad():
                    q_preds = self.model_q(x_batch).squeeze()
                ...
                action, _, _ = self.model.step(torch.from_numpy(states), torch.from_numpy(legal_index))
                
                # self.logger.info(f"{action = }")
                # eps = 0.02 

                # action_idx = torch.argmax(q_preds).item()
                action_idx = indexs[action]
